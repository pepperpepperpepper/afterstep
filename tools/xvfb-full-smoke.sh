...
"$AFTERSTEP_BIN" -p "$HOME/.afterstep" -g "$AFTERSTEP_CONFIG_DIR" -l "$AS_SMOKE_LOG" -V 5 &
as_pid=$!
...
# Allow modules/autoexec a bit of time to start.
sleep 6
...
timeout 30s xvfb-run -a -s "-screen 0 1024x768x24" env \
  HOME="$tmp_home" \
